...
% is the following sentence needed?
...
% maybe start with "Typma will be implemented OCaml & Coq - functional programming languages \textcite{OCaml}"
...
% this phrasing is awkward, maybe change to "a product type is ``tuples or records...""
...
% awkward. maybe change to "To perform case analysis on a term of an algebraic data type pattern matching is used to determine what to do for each variant."
For code that deals with different versions of an algebraic data type, pattern matching
% don't need to mention let-expressions here.
...
% some languages have both null & pattern matching (scala) :(.
...
% let expressions are only one place where patterns occur. the "match e with | case1 -> e1 ... | casen -> en"
% is another. don't need to mention let-expressions here
...
% an abstract syntax tree (a data-type to represent a program)
the code as a tree.
% "a parser can be written", but why mention this?
Also, a parser can written without the use of tokens from a lexer.
% This project will use an abstract syntax tree (AST)
...
% maybe use notation for your example?
...
% I'm not sure the way you have written your example makes the difference between small-step & big-step clear.
% maybe doing a proof with inference rules of the evaluation in each would make the difference clear.
...
% maybe a too lengthy? "Typma is inspired by esolangs J.A.V.A & SimPLe"
...
% maybe say "interpreter" instead of "compiler"?
% awkawrd, maybe change phrasing to "the interpreter will decide the type at runtime"
...
% the typedevil mention seems out of nowhere but idk
...
% im confusued. was the experiment ever performed & if so what were the results?
% the way this is phrased makes it sound like the experiment was done only once
% what claim or hypothesis did they have?
...
% be specific and say what character this is
...
% maybe say you will prove big-step & small-step semantics are equivalent?
...
% with a proof that the evaluation function is equivalent to the inductive proposition representing the formal semantics
...
% "Assuming no work is done directly on the project"?
% awkward
...
\item Nov 15: Poster due
  ...
  % is the interpreter not already an interpreter for typma
  % does "adding typma stuff" mean adding random typing?
